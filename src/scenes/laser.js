...
/**
 * The blast a shot delivers to one sphere.
 *
 * Energy comes off the beam's *axis*, not its origin: a line charge
 * rather than a point one, so what it drives is a cylinder opening
 * outward. The impulse is therefore perpendicular to the beam, along
 * the shortest line from the axis to the sphere's centre.
 *
 * Falls off with distance from the axis and does nothing behind the
 * eyes, which is what keeps a shot from shoving the cluster it is
 * pointing away from.
 *
 * @returns {number} impulse magnitude; the direction is written to `out`
 */
export function blastAt(origin, dir, centre, out, radius, strength) {
  const ox = centre[0] - origin[0], oy = centre[1] - origin[1], oz = centre[2] - origin[2];

  // How far along the beam the sphere sits. Behind the muzzle is a miss.
  const t = ox * dir[0] + oy * dir[1] + oz * dir[2];
  if (t <= 0) return 0;

  // The shortest vector from the axis out to the centre.
  let rx = ox - dir[0] * t, ry = oy - dir[1] * t, rz = oz - dir[2] * t;
  const r = Math.hypot(rx, ry, rz);
  if (r > radius) return 0;

  /* A sphere sitting exactly on the axis has no outward direction to be
     pushed in. Rather than leave it untouched — the one case where the
     shot is best aimed — it is nudged along the beam instead. */
  if (r < 1e-3) {
    out[0] = dir[0]; out[1] = dir[1]; out[2] = dir[2];
    return strength;
  }

  out[0] = rx / r; out[1] = ry / r; out[2] = rz / r;

  // Smooth to zero at the edge, so a sphere drifting out of the blast
  // does not have its impulse cut off mid-flight.
  const f = 1 - r / radius;
  return strength * f * f;
}
