  ...
  params: [
    ...
    { id: 'hintWater', type: 'hint',
      text: '水是**一個數字**：水面的高度。它是一個水平面，射線用一次除法就碰到它，'
        ...
        + '尾巴也不再是舵而是螺旋槳：**兩下划水掃一整圈 57° 的錐面**（從後面看順時鐘），'
        + '而且**彎曲是沿著路徑而不是橫過它**——尖端在的地方就是根部一會兒之前在的地方，'
        + '那是運動學不是動力學：交給彈簧鏈去做只會共振（實測放大三倍），彎出來離路徑七十度。'
        + '頭則隨著划水**往上點**——振盪取自被抬進 0..1 的正弦，所以牠的鼻子不會低於水面該有的高度。'
