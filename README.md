...
所以這裡用的是**加法**：能量高的地方把表面往內推，而因為衰減是一團平滑的高斯，
表面是**逐漸收細到消失**而不是被切開。消散量超過當地厚度的地方，表面就直接不存在了 ——
星環只有 0.075 厚，所以那裡會裂開一個缺口，再隨能量排空長回來。

加法還有一個好處：它的梯度是「振幅除以寬度」而不是「振幅乘以波數」，
所以它直接吃前面那套「步長從場扭曲推導」的機制，不需要任何特例。
