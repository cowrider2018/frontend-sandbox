...
src/
  ...
  core/             ← 引擎（與場景無關）
    ...
    agent.js        「游者」：轉向行為 + IK 脊椎，四個場景共用同一隻
...
| # | 場景 | 核心技術 | 游者在裡面做什麼 |
|---|------|----------|------------------|
| 01 | **旋度噪聲流場** | GPGPU：位置／速度存在 RGBA32F 貼圖，每幀兩道全螢幕 pass 積分整個系統。渲染用 vertex pulling（`gl_VertexID` → texel fetch），**沒有任何頂點屬性**，百萬粒子的頂點頻寬是 0 bytes。四分之一解析度 bloom。 | 尾流：沿行進方向推開介質，並在該方向上甩出渦環（那個 `cross` 項），在粒子雲裡犁出一條通道。本體是一條發光緞帶，交給 bloom。 |
| 02 | **Navier–Stokes 流體** | Stam 的 stable fluids：半拉格朗日反向追蹤對流（任意 dt 都無條件穩定）→ 渦度強化 → Jacobi 迭代解壓力泊松方程 → 投影成無散度場。每幀 8–40 道 pass。 | 牠**就是**攪動者。沿脊椎前三分之一以遞減強度注入速度與染料，渦度強化再把它捲成漩渦。你不直接碰流體，你牽著一隻動物走，尾流就是畫。 |
| 03 | **SDF 光線行進** | 整個 3D 場景 **0 頂點 0 三角形**：一道 fragment shader 用 sphere tracing 走進隱式曲面。smooth-min 融合、IQ 軟陰影、5 取樣環境遮蔽、一次鏡面反射彈跳、jitter + 時間累積的免費 TAA。 | 牠是真正的幾何：11 個逐段收細的圓錐膠囊直接進 `map()`，因此自動獲得真實的軟陰影、環境遮蔽與鏡面反射。包圍球提前退出讓這 11 個距離函式付得起。 |
| 04 | **Gray–Scott 反應擴散** | 兩行偏微分方程長出珊瑚、迷宮、細胞分裂與孤立子。九點拉普拉斯算子、每幀 16 個子步、環面邊界（`REPEAT` wrap，所以圖樣可無縫平舖）。 | 頭部把化學場推回乾淨基質，切開一條通道 —— 而乾淨基質是這個模型的**穩定**狀態，所以通道不會自己癒合。身體中段再往通道裡撒下菌落，尾跡於是開花。 |

---

## 游者

四個場景共用同一隻生物，牠住在 App 層而不屬於任何場景 —— 換場景時把同一隻、同樣的位置與動量、正在轉彎到一半的游者交給下一個渲染器。

牠活在一個「正規空間」（原點附近半徑 1.9 的球）裡，每個場景把那個空間映射到自己的座標：3D 場景映到世界單位，2D 場景映到 UV。指標的目標點則反向穿過各場景自己的相機基底換算回來 —— 所以「牠跟著我的游標」在鏡頭繞了半圈之後仍然成立，這是天真的螢幕座標映射做不到的。

內部刻意拆成兩個系統：

**轉向**決定頭要去哪。Reynolds 的作法：算出期望速度，然後朝它加速。速度同時有上限與**下限** —— 會突然停住的東西讀起來是粒子，不是動物。

**脊椎**跟著走。每個節點被拉到剛好落在前一個節點後方一段距離處，也就是 follow-the-leader 反向運動學。身體沒有被模擬，牠純粹是「頭去過哪裡」的結果，所以永遠不會打結，也不需要解算器。

擺動是疊在**顯示用副本**上的橫向行波，尾拍頻率隨速度上升 —— 這是最強烈讀作「在游泳」而不是「被拖著走」的單一線索。把擺動寫回脊椎鏈會讓抖動回饋進約束裡，身體會慢慢把自己鋸開。
...
`--interact` 的 19 項檢查涵蓋：intro 關閉、指標事件真的進到畫布、拖曳注入染料、`Ctrl`+`K`
開啟面板、模糊搜尋過濾、面板導航、游者朝指標轉向、游者跨場景保留動量、拖曳繞行鏡頭、
數字鍵切換場景、參數往返 URL、暫停真的停住時鐘、`.` 前進一幀、禪模式、快捷鍵表、
tab 點擊、指示器追蹤，以及全程零 console error。
