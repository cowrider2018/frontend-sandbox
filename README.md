...
**星團照到貓。** 貓朝太陽射一條軟陰影射線，穿過**同一個**距離場 —— 這正是星團的形狀
從 march.js 搬進 `scenes/cluster.js` 的原因：marcher 不再是它唯一的讀者，而兩份一模一樣的公式，
就是這個場景在別處明確警告過、會先同意一週再悄悄走偏的東西。只有形狀搬過去；
漣漪、表面雜訊、質量消散留在 marcher 裡，因為它們只把表面推動幾公分，外面的人看不出那種差別。

射線從開闊地面出發時幾乎立刻離開星團的包圍球，所以在野外它的成本就是一次 miss；
背對光的面則根本不發射線。另外加了距離霧，**連黑邊也要一起霧化**，否則貓會整個化掉卻留著邊。

環境光下限從 0.42 降到 0.30：marcher 自己的環境光只有主光的十分之一，
一隻在陰影裡還有近半亮度、周圍卻幾乎全黑的貓，正是貼紙的行為。
