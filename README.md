...
一圈熾熱的環沿著表面擴散出去。把「撞擊坑」拉大，那一擊會在球體上打穿一個洞、
或把星環整個炸斷。
...
**撞擊坑。** 波只能把表面推開一點點，再深就不再是距離場了。**移除材料是另一種運算**：
`max(d, −球)` 是精確的布林減法，而兩個 1-Lipschitz 場的 max 仍是 1-Lipschitz ——
所以它想挖多深都行，追蹤器不會失準。夠深就會在球體上開一個貫穿的洞，或把星環整個截斷。
坑會迅速張開、再隨著波的能量耗盡癒合。

這個減法放在**形狀層**而不是完整層：洞對陰影與遮蔽射線也必須是洞，
